...
def isUsernameFree(name):
    #this function is to check username is free in contacts or not
    global username_array
    global username
...
def passFriends(conn):
    ...
    for connection in conn_array:
        if conn != connection:
            ...
            conn.send(formatNumber(len(connection.getpeername()[0])).encode())
...
def contacts_window(master):
    ...
    dBut.pack(side=LEFT)
    aBut = Button(buttons, text="Add",command=lambda: contacts_add(listbox, cWindow))
    aBut.pack(side=LEFT)
...
def contacts_add(listbox, master):
    ...
    go = Button(aWindow, text="Add", command=lambda:contacts_add_helper(name.get(), ip.get(), port.get(),aWindow, listbox))
...
connection_menu.add_command(label="contacts",command=lambda:contacts_window(root))
